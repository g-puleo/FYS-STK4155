...
def generate_reults():
    ...
    for met in methods:
        lamb_val = 0.1
        #MSE plot example
        degrees_list, MSE_train_list, MSE_test_list, bias, variance = Solver(x_, y_, z_, Nx_, Ny_, met, lamb = lamb_val, useBootstrap=False, useCrossval=False, maxdegree = maxdeg)
        plotfnc.MSE_plot(degrees_list, MSE_train_list, MSE_test_list, title=f"{met}", savefig = True)

    #Bias var plot example
    degrees_list, MSE_train_list, MSE_test_list, bias, variance = Solver(x_, y_, z_, Nx_, Ny_, OLS, useBootstrap=True, useCrossval=False, maxdegree = maxdeg)
    ...
    for i in range(len(lambda_vals)):
        degrees_list, MSE_train_list, MSE_test_list, _, _ =  Solver(x_, y_, z_, Nx_, Ny_, Ridge, useBootstrap=useBoot, useCrossval=useCross, lamb=lambda_vals[i], mindegree = mindeg, maxdegree = maxdeg)
